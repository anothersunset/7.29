...
问题四使用按行动态规划计算每个格子的可行取值，再进行行列传播和最少未知格分支。该方法不需要显式生成单行的数十万条候选序列。

```bash
python -m scripts.solve_problem4 data/附录2-第四问数据.txt \
  --output-dir outputs/problem4
```

输出完整解空间JSON、结果工作簿、代表解图和公共矩阵图。公共矩阵中红色格为多解空格。
...
- 正式实验中 `E[B|rho]` 的离散峰值出现在 `rho=0.20`，均值为 `72.21`；
- 首个观测到唯一解概率不低于50%的网格点为 `rho=0.50`，概率为 `54.5%`；
- 问题四50×50实例共有24个完整解、30个多解空格，全部行列编码验证通过；
- 问题四恢复图像为世界地图，主要呈现美洲、欧亚大陆、非洲和大洋洲轮廓。
