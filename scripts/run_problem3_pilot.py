"""Run the reproducible Problem 3 fixed-density pilot experiment."""
...
def run(n:int,ratios:list[float],trials:int,seed:int,output_dir:Path):
    ...
    with (output_dir/"pilot_results.csv").open("w",newline="",encoding="utf-8-sig") as f:w=csv.DictWriter(f,fieldnames=records[0].keys());w.writeheader();w.writerows(records)
    with (output_dir/"pilot_summary.csv").open("w",newline="",encoding="utf-8-sig") as f:w=csv.DictWriter(f,fieldnames=summaries[0].keys());w.writeheader();w.writerows(summaries)
    (output_dir/"pilot_summary.json").write_text(json.dumps({"seed":seed,"pilot_only":True,"summaries":summaries},ensure_ascii=False,indent=2),encoding="utf-8");return summaries
def main():
    p=argparse.ArgumentParser();p.add_argument("--n",type=int,default=10);p.add_argument("--ratios",type=float,nargs="+",default=[.1,.3,.5,.7,.9]);p.add_argument("--trials",type=int,default=20);p.add_argument("--seed",type=int,default=20260727);p.add_argument("--output-dir",type=Path,default=Path("outputs/problem3"));a=p.parse_args()
    if a.n<=0 or a.trials<=0 or any(not 0<=r<=1 for r in a.ratios):p.error("invalid parameters")
    run(a.n,a.ratios,a.trials,a.seed,a.output_dir)
